...
from materials_mvp.environment import ExplorationEnvironment, build_dimensionality_audit, save_audit, save_summary
...
def main() -> None:
    ...
    audit_rows, audit_summary = build_dimensionality_audit(rows, config)
    save_audit(audit_rows, ROOT / "artifacts" / "dimensionality_audit.csv")
    summaries = []
    ...
    save_summary({"strategy_results": summaries, "dimensionality_audit": audit_summary}, ROOT / "artifacts" / "summary.json")
    ...
    print(f"Dimensionality audit: {audit_summary['uncertain_2d_count']} uncertain and {audit_summary['non_2d_count']} non-2D records excluded from training but retained for statistics")
